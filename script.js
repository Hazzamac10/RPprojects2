...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Expose smooth scroll globally for inline onclick usage
    const app = window;
    ...
    // Minimal lightbox for portfolio images
    initPortfolioLightbox();

    // Render hero PDF (New Build) into canvas with pdf.js to avoid scrollbars
    renderHeroPdfToCanvas(
...
// FormSubmit form handler (improves UX)
...
function initPreloader() {
    ...
    const hide = () => {
        ...
        // Ensure initial above-the-fold reveals animate immediately
        triggerInitialReveals();
    };

    // Safety timeout in case the 'load' event is delayed
    const maxWait = setTimeout(hide, 2500);
...
async function renderHeroPdfToCanvas(pdfUrl, canvas) {
    ...
    try {
        ...
        const desiredHeight = canvas.clientHeight || 460;
        ...
        console.error('Failed to render hero PDF:', e);
    }
}

function initPortfolioLightbox() {
    const images = Array.from(document.querySelectorAll('.portfolio-img'));
    ...
    if (!overlay) {
        ...
        overlay.innerHTML = `
            <div class="lightbox-content">
                ...
                <img class="lightbox-image" alt="Expanded project image" />
            </div>
        `;
